...
public class ZitiDriver implements java.sql.Driver {
  ...
  private static Set<String> ztConfigs = new HashSet<>();
  private static Set<BaseZitiDriverShim> configuredShims = new HashSet<>();

  public static final String ZITI_JSON = "ztIdentityFile";
  public static final String ZITI_IDENTITY = "ztIdentity";
  public static final String ZITI_KEYSTORE = "ztKeystore";
  public static final String ZITI_KEYSTORE_PASSWORD = "ztKeystorePassword";
  public static final String ZITI_DRIVER_URL_PATTERN = "ztDriverUrlPattern";
  public static final String ZITI_DRIVER_CLASSNAME = "ztDriverClassname";
  public static final String ZITI_DRIVER_FEATURES = "ztDriverFeatures";
  public static final String ZITI_WAIT_FOR_SERVICE_NAME = "ztWaitForService";
  public static final String ZITI_WAIT_FOR_SERVICE_TIMEOUT = "ztWaitForServiceTimeout";
  ...
  public boolean acceptsURL(String url) throws SQLException {
    return null != url && (url.startsWith("zdbc") || url.startsWith("jdbc:zt:"));
  }
  ...
  private String toProviderUrl(String zdbcUrl) {
    ...
    } else if (zdbcUrl.startsWith("jdbc:zt")) {
      result = zdbcUrl.replaceFirst("jdbc:zt","jdbc");
    } else {
      result = zdbcUrl;
    }
  ...
  public DriverPropertyInfo[] getPropertyInfo(String url, Properties info) throws SQLException {
    ...
    try {
      ...
      DriverPropertyInfo driverUrlPattern = new DriverPropertyInfo(ZITI_DRIVER_URL_PATTERN, "^jdbc.zt:<database>.*");
      ...
      DriverPropertyInfo ztFeatures = new DriverPropertyInfo(ZITI_DRIVER_FEATURES, "seamless,nioProvider");
  ...
  private synchronized void setupZiti(BaseZitiDriverShim shim, Properties info) {
    ...
    if (ztConfigs.contains(info.getProperty(ZITI_JSON))
        || ztConfigs.contains(info.getProperty(ZITI_KEYSTORE))
        || ztConfigs.contains(info.getProperty(ZITI_IDENTITY))) {
    ...
    if (info.containsKey(ZITI_JSON)) {
      ...
      ztConfigs.add(info.getProperty(ZITI_JSON));
      ...
      ztConfigs.add(info.getProperty(ZITI_KEYSTORE));
    } else if (info.containsKey(ZITI_IDENTITY)) {
      log.finer(() -> String.format("Found zt identity connection properties."));
    ...
    if (!configuredShims.contains(shim) && requiresFeature(shim, seamless) && !Ziti.isSeamless()) {
      throw new ShimException(String.format(
          "Ziti JDBC configuration error. JDBC driver %s requires zt seamless mode, but it is not enabled",
