# ZDBC
A repository to provide jdbc driver configurations for access to databases via zt
...
## Driver Features
Each JDBC driver needs specific zt features in order to work. This table attempts to capture which each driver requires
...
# How it works
The zdbc driver registers with `java.sql.DriverManager` when the zdbc wrapper jar is included in the application. The Ziti JDBC wrapper checks each database URL to see if it starts with `zdbc` or `jdbc:zt`. If it does, then the wrapper accepts the connection request, configures Ziti,  configures the driver,  and then delegates to the driver to establish a database connection over the Ziti network fabric.

# Your Ziti network identity
The zdbc driver needs your zt network identity to connect. There are three ways to give your identity to the driver.
1. PKCS12 keystore and password. Provided via the JDBC properties `ztKeystore` and `ztKeystorePassword`
1. Path to a `.json` file containing your zt identity. This is the file created during enrollment. Provided via the JDBC property `ztIdentityFile`
1. A zipped and base64encoded string provided via the JDBC property `ztIdentity`. The zdbc jar provides a main method that can encode the json zt identity file into this format. This option is intended to be used for support tooling or business application deployments where you cannot write to the server file system.

# Example of integrating into developer tools
## Requirements
1. A Ziti network and database: (https://github.com/hanzozt/zt-sdk-jvm/blob/main/samples/jdbc-postgres/cheatsheet.md) 
...
1. Ziti ZDBC wrapper full jar: (this project): (https://search.maven.org/search?q=g:org.hanzozt&a=zt-jdbc&l:full)

## Step by Step
1. Configure a zt network and postgres database following the cheatsheet <https://github.com/hanzozt/zt-sdk-jvm/blob/main/samples/jdbc-postgres/cheatsheet.md> 
1. Copy the Ziti all-in-one jar into the Squirrel-Sql `lib` folder

> ls $SQUIRREL_HOME\lib | grep zt <br/>
    zt-0.25.1-full.jar <br/>
    zt-jdbc-0.25.1.jar
...
1. Create a PostgreSQL alias with the following values
    ...
    1. Driver: `PostgreSQL`
    1. URL: `jdbc:zity:postgresql://ztfied-postgres/simpledb`
    1. User Name: `postgres`
    1. Password: `postgres`
    1. The Ziti Identity file is provided via driver properties.  Click the Properties button and set the ztIdentityFile property to the java-identity.json file created during the network setup
...
* [samples/postgresql/README.md](https://github.com/hanzozt/zt-sdk-jvm/blob/main/samples/jdbc-postgres/cheatsheet.md): Postgresql example using the JDBC DriverManager to connect to a dark database.
* [samples/spring-jpa/README.md](https://github.com/hanzozt/zt-sdk-jvm/tree/main/samples/zt-jdbc-spring-jpa): A learning exercise adding spring-data-jpa and spring-data-rest to host a dark http server that connects to a dark postgres database
