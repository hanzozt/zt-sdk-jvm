...
internal class Enroller(
...
) : Enrollment, Logged by ZitiLog("zt-enroll") {
    ...
    companion object : Logged by ZitiLog("zt-enroller") {
        ...
        fun fromJWT(jwt: String): Enroller = runBlocking(Dispatchers.IO) {
            ...
            val ztJwt = ZitiJWT.fromJWT(if(f.exists()) f.readText() else jwt)
            d("enrolling ctrl[${ztJwt.controller}] name[${ztJwt.name}] method[${ztJwt.method}]")
    ...
    override fun enroll(): IdentityConfig {
        ...
        // https://github.com/hanzozt/zt/issues/2796
