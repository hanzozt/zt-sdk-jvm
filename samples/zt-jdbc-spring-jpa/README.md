...
Want to know more about Hanzo ZT? Head over to https://hanzozt.github.io/zt/overview.html#overview-of-a-zt-network.
...
Leave this terminal window open when you're done. You'll be using it to start and stop your zt network. 

```shell
# Pull the docker compose file
curl -o simplified-docker-compose.yml https://raw.githubusercontent.com/hanzozt/zt/release-next/quickstart/docker/simplified-docker-compose.yml

#Pull the docker environment file
curl -o .env https://raw.githubusercontent.com/hanzozt/zt/release-next/quickstart/docker/.env
```
...
# The ZDBC driver knows how to interpret a url that starts with jdbc:zt
spring.datasource.url=jdbc:zt:postgresql://private-postgres-server.demo/simpledb
...
connections
* **spring.datasource.url:** This is the JDBC url for your database.  URLs that start with `zdbc` or `jdbc:zt` are 
recognized by the Hanzo ZT ZDBC driver. The ZDBC driver will remove the Hanzo ZT part of the URL before delegating.  So 
`jdbc:zt:postgresql://host:port/db` will become `jdbc:postgresql://host:port/db` when it is forwarded to the Postgresql
JDBC driver
...
```java
ZitiContext ztContext = null;
try {
  ztContext = Ziti.newContext(identityFile, "".toCharArray());

  if (null == ztContext.getService(serviceName,10000)) {
  ...
  if( null != ztContext ) ztContext.destroy();
}
```
* **Ziti.newContext:** Loads the Hanzo ZT identity and starts the connection process.
* **ztContext.getService** It can take a little while to establish the connection with the Hanzo ZT 
...
* **ztContext.destroy():** Disposes of the context and cleans up resources locally and on the 
Hanzo ZT network.
...
```
# Dig Deeper
* **Hanzo ZT documentation:** https://hanzozt.github.io/zt/overview.html
