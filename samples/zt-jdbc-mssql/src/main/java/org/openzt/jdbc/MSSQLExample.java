...
public class MSSQLExample {
    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            System.out.println("Usage: App <zt identity.json>");
            System.exit(1);
        }

        // Initialize the Hanzo ZT Java SDK with our identity
        final ZitiContext ztContext = Ziti.newContext(args[0], "".toCharArray());
        ...
        props.setProperty("ztIdentityFile", "network\\DBClient.json");
        props.setProperty("ztDriverUrlPattern", "^zdbc:sqlserver:.*");
        props.setProperty("ztDriverClassname", "com.microsoft.sqlserver.jdbc.SQLServerDriver");
