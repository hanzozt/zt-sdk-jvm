...
object HttpSample {
    ...
    class client: CliktCommand("client") {
        val service by option("-s","--service",
            help = "zt service to dial (not needed if hostname:port in given URL is intercepted)")
        ...
        override fun run() {
            ...
            val bootstrap = Bootstrap()
                .group(loopGroup)
                .resolver(ZitiResolverGroup(zt))
                .channelFactory(ZitiChannelFactory(zt))
    ...
    class server: CliktCommand("server") {
        val service by argument("service", "zt service to bind to")

        override fun run() {
            val server = ServerBootstrap()
                .group(DefaultEventLoopGroup())
                //.group(NioEventLoopGroup())
                .channelFactory(ZitiServerChannelFactory(zt))
    ...
    object cli: CliktCommand("http-sample", name = "http-sample") {
        ...
        override fun run() {
           zt = Ziti.newContext(idFile, charArrayOf())
        }
    }
